...
class StarSystem:
    ...
    law_lvl = 0
    tech_lvl = 0
    def __init__(self, mainworldname, size, atm, hyd, pop, gov, law_lvl, tech_lvl):
        self.size = size
        self.atm = atm
        self.hyd = hyd
        self.pop = pop
        ...
        self.mainworldname = mainworldname
...
print ("===========randomly generate star system objects and add to a list ==============================")    
...
for key, value in zip(upp_dict.keys(), upp_dict.values()):
        ...
        mainworldname = key
        size = str(getattr(my_upp, 'size'))
        atm = str(getattr(my_upp, 'atm'))
        hyd = str(getattr(my_upp, 'hyd'))
        pop = str(getattr(my_upp, 'pop'))
        gov =  str(getattr(my_upp, 'gov'))
        law_lvl = str(getattr(my_upp, 'law_lvl'))
        tech_lvl = str(getattr(my_upp, 'tech_lvl'))
        # create an object of StarSystem
        start_system = StarSystem(mainworldname, size, atm, hyd, pop, gov, law_lvl, tech_lvl)
        # add the object to the list (this list is a list of objects of StarSystem class)
        star_system_list.append(StarSystem)
